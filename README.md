...
2. **发送消息**
   ...
   - 支持Markdown格式回复
   - 代码自动高亮
   - 支持命令：`/sys 你的指令`（动态更新系统指令，不影响学习背景）

3. **学习进度引导（可在设置中关闭）**
   - 阶段1：首次交互可选择学习进度（新手/初级/中级/高级/专家）
   - 阶段2：输入当前学习基础，AI会据此定制回答深度
   - 阶段3：阶段信息自动写入会话System Prompt
   - 主界面提供单次可关闭提示气泡

4. **窗口操作**
   - **拖拽**：按住标题栏移动
   - **拉伸**：拖拽右下角⋰图标
   - **更宽窗口**：默认宽度增大，可向右拉伸到更大尺寸
   - **最小化**：点击 ━ 按钮
   - **关闭**：点击 ✕ 按钮
...
6. **设置项**
   - 点击聊天窗口顶部 ⚙ 按钮可打开设置
   - 可配置学习引导开关、默认System Prompt、字体大小、对话背景风格
